...
// BOOST_CLASS_EXPORT_IMPLEMENT(orbslam3::KannalaBrandt8)

namespace orbslam3 {
// BOOST_CLASS_EXPORT_GUID(KannalaBrandt8, "KannalaBrandt8")

cv::Point2f
KannalaBrandt8::project(const cv::Point3f& p3D)
{
...
Eigen::Vector2d
KannalaBrandt8::project(const Eigen::Vector3d& v3D)
{
...
Eigen::Vector2f
KannalaBrandt8::project(const Eigen::Vector3f& v3D)
{
...
Eigen::Vector2f
KannalaBrandt8::projectMat(const cv::Point3f& p3D)
{
...
float
KannalaBrandt8::uncertainty2(const Eigen::Matrix<double, 2, 1>& p2D)
{
...
Eigen::Vector3f
KannalaBrandt8::unprojectEig(const cv::Point2f& p2D)
{
...
cv::Point3f
KannalaBrandt8::unproject(const cv::Point2f& p2D)
{
...
bool
KannalaBrandt8::ReconstructWithTwoViews(const std::vector<cv::KeyPoint>& vKeys1,
...
bool
KannalaBrandt8::epipolarConstrain(GeometricCamera* pCamera2,
...
bool
KannalaBrandt8::matchAndtriangulate(const cv::KeyPoint& kp1,
                                    const cv::KeyPoint& kp2,
...
float
KannalaBrandt8::TriangulateMatches(GeometricCamera* pCamera2,
                                   const cv::KeyPoint& kp1,
                                   const cv::KeyPoint& kp2,
                                   const Eigen::Matrix3f& R12,
                                   const Eigen::Vector3f& t12,
...
std::ostream&
operator<<(std::ostream& os, const KannalaBrandt8& kb)
{
...
std::istream&
operator>>(std::istream& is, KannalaBrandt8& kb)
{
  float nextParam;
...
void
KannalaBrandt8::Triangulate(const cv::Point2f& p1,
                            const cv::Point2f& p2,
...
bool
KannalaBrandt8::IsEqual(GeometricCamera* pCam)
{
