...
// BOOST_CLASS_EXPORT_IMPLEMENT(orbslam3::Pinhole)
...
cv::Point2f
Pinhole::project(const cv::Point3f& p3D)
{
...
Eigen::Vector2d
Pinhole::project(const Eigen::Vector3d& v3D)
{
  Eigen::Vector2d res;
...
Eigen::Vector2f
Pinhole::project(const Eigen::Vector3f& v3D)
{
  Eigen::Vector2f res;
  res[0] = mvParameters[0] * v3D[0] / v3D[2] + mvParameters[2];
  res[1] = mvParameters[1] * v3D[1] / v3D[2] + mvParameters[3];
...
Eigen::Vector2f
Pinhole::projectMat(const cv::Point3f& p3D)
{
...
float
Pinhole::uncertainty2(const Eigen::Matrix<double, 2, 1>& p2D)
{
  return 1.0;
}

Eigen::Vector3f
Pinhole::unprojectEig(const cv::Point2f& p2D)
{
...
cv::Point3f
Pinhole::unproject(const cv::Point2f& p2D)
{
...
Pinhole::projectJac(const Eigen::Vector3d& v3D)
{
...
bool
Pinhole::ReconstructWithTwoViews(const std::vector<cv::KeyPoint>& vKeys1,
...
bool
Pinhole::epipolarConstrain(GeometricCamera* pCamera2,
...
std::ostream&
operator<<(std::ostream& os, const Pinhole& ph)
{
...
std::istream&
operator>>(std::istream& is, Pinhole& ph)
{
  float nextParam;
...
bool
Pinhole::IsEqual(GeometricCamera* pCam)
{
